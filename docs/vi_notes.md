...
| `DAQ/Waveform/*`, `DAQ/Control Modules/FPGA/*`, `Image Acq/DCAM/*` — the cycle-time set | How Cam exp / Cycle time / Custom Cycle Time behave. **21 claims, all UNVERIFIED** — the adversarial pass died on a usage limit. Read the write-up's caveat before trusting any of it. | many hidden | `louisxiv_cycle_time_semantics.md` |
