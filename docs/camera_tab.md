...
| FOV X/Y | `pixels * pixel size`, pixel size = 6.5 um * binning / mag 30 = 0.2167 um. The 6.5 um pitch is CONFIRMED 2026-09-05 from the `"Orca4.0"` case of `DCAM CCD Pixel sizes.vi` (hidden frames; the `"Orca2.8"` case is 3.63). We therefore show 443.7 um at full frame where LouisXIV shows 444.0 -- the inputs agree, so the 0.3 um is LouisXIV's indicator formatting (443.733 to 3 significant digits), not a different calculation. Not chased further. | `DCAM CCD Pixel sizes.vi`, `SPIMProject.ini [Detection optics]` |
